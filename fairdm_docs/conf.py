...
def _apply_theme_config(theme: str, metadata: ProjectMetadata) -> dict[str, Any]:
    ...
    if theme == "pydata_sphinx_theme":
        ...
        # sphinx_book_theme options (default). The repository, issue and edit
        # buttons all read the same address; a portal that has declared none
        # gets none of them, rather than buttons the theme cannot resolve.
...
# API documentation from a portal's own source needs the portal's package
# list, which the build does not yet read (docs/ROADMAP.md R4). Loading
# autodoc2 with nothing to document produces a warning the developer has no
# way to act on, so it stays out until R4 wires package discovery:
# autodoc2_packages = [
